...
struct TTEntry {
    ...
    TTEntry()
        : zobrist(0ull), depth(0), age(0), type(EntryType::NONE), move_code(0),
          value(0) {
    }
    TTEntry(const Position &pos, EntryType type, int8_t depth, Move move,
            Value value)
        : zobrist(pos.hash()), depth(depth), age(0), type(type),
          move_code(move.move()), value(value) {};
...
class TranspositionTable {
  ...
  public:
    TranspositionTable() : size(0), occupied(0), generation(0) {
        db = nullptr;
    }
    ~TranspositionTable() {
        delete[] db;
    ...
    void store(const Position &pos, EntryType type, int8_t depth, Move move,
               Value value);
    TTEntry *probe(const Position &pos);
    std::pair<bool, Value> lookupEval(const Position &pos, int8_t depth,
                                      int8_t plyFromRoot, Value alpha,
                                      Value beta);
};
