...
public:
    ...
    inline bool isValid() const { return m_value != -32768; }
    inline bool isMate() const {
        return isValid() && (m_value >= MATE_VALUE_THRESHOLD || m_value <= -MATE_VALUE_THRESHOLD);
    }

    constexpr static Value mateIn(int ply) { return Value(32767 - ply); }

    constexpr static Value matedIn(int ply) { return Value(-32767 + ply); }
    constexpr static Value none() { return Value(-32768); }
    ...
    inline Value operator+(const Value& rhs) const { return Value(m_value + rhs.m_value); }
    inline Value operator+(const int rhs) const { return Value(m_value + rhs); }
    inline Value operator-(const Value& rhs) const { return Value(m_value - rhs.m_value); }
    inline Value operator-(const int rhs) const { return Value(m_value - rhs); }
    inline Value operator*(const Value& rhs) const { return Value(m_value * rhs.m_value); }
    inline Value operator*(const float rhs) const { return Value(static_cast<int>(m_value * rhs)); }
    inline Value operator*(const int rhs) const { return Value(m_value * rhs); }
    inline Value operator/(const int rhs) const { return Value(m_value / rhs); }
    inline Value operator~() const { return Value(-m_value); }
    inline       operator int() const { return m_value; }

    inline bool   operator>(const Value& rhs) const { return m_value > rhs.m_value; }
    inline bool   operator>=(const Value& rhs) const { return m_value >= rhs.m_value; }
    inline bool   operator<(const Value& rhs) const { return m_value < rhs.m_value; }
    inline bool   operator<=(const Value& rhs) const { return m_value <= rhs.m_value; }
    inline bool   operator==(const Value& rhs) const { return m_value == rhs.m_value; }
...
public:
    ...
    Value fuse(int k, int N = ALL_GAME_PHASES) const {
        ...
        short v  = std::max(-MATE_VALUE_THRESHOLD, std::min(MATE_VALUE_THRESHOLD, mg + eg));
        return Value(v);
    }

    inline Score  operator+(const Score& rhs) const { return Score(mg + rhs.mg, eg + rhs.eg); }
    inline Score  operator-(const Score& rhs) const { return Score(mg - rhs.mg, eg - rhs.eg); }
    inline Score  operator*(const int rhs) const { return Score(mg * rhs, eg * rhs); }
    inline Score  operator/(const int rhs) const { return Score(mg / rhs, eg / rhs); }
    inline Score  operator~() const { return Score(~mg, ~eg); }
