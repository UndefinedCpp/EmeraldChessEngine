...
public:
    ...
    uciToMove(const Board& board, const std::string& uci) noexcept(false) {
        if (uci.length() < 4) { return Move::NO_MOVE; }

        Square source = Square(uci.substr(0, 2));
        Square target = Square(uci.substr(2, 2));

        if (!source.is_valid() || !target.is_valid()) { return Move::NO_MOVE; }
    ...
    parseSan(const Board& board, std::string_view san, Movelist& moves) noexcept(false) {
        if (san.empty()) { return Move::NO_MOVE; }
        ...
        if (info.castling_short || info.castling_long) {
            ...
            throw SanParseError(
                "Failed to parse san. At step 2: " + std::string(san) + " " + board.getFen());
#endif
        }
        ...
        for (const auto& move : moves) {
            ...
            if (move.to() != info.to || move.typeOf() == Move::CASTLING) { continue; }
            ...
            // Handle en passant moves
                ...
                if (move.from().file() != info.from_file) { continue; }
            }
            // Handle moves with specific from square
            else if (info.from != Square::NO_SQ) {
                if (move.from() != info.from) { continue; }
            }
            ...
            if (foundMatch) {
#ifndef CHESS_NO_EXCEPTIONS
                throw AmbiguousMoveError(
                    "Ambiguous san: " + std::string(san) + " in " + board.getFen());
#endif
            }
...
#ifndef CHESS_NO_EXCEPTIONS
            throw SanParseError(
                "Failed to parse san, illegal move: " + std::string(san) + " " + board.getFen());
#endif
        }
    ...
    static bool isUciMove(const std::string& move) noexcept {
        ...
        if (move.size() == 5) { is_uci = is_uci && is_promotion(move[4]); }
...
private:
    ...
    [[nodiscard]] static SanMoveInformation parseSanInfo(std::string_view san) noexcept(false) {
        ...
        if (san[0] == 'O' || san[0] == '0') {
            ...
            if (info.piece == PieceType::NONE) { throw_error = true; }
        }
    ...
    static void moveToRep(Board board, const Move& move, std::string& str) {
        ...
        if (pt != PieceType::PAWN) { appendPieceSymbol(pt, str); }
    ...
    resolveAmbiguity(const Board& board, const Move& move, PieceType pieceType, std::string& str) {
        ...
        if (hasAmbiguousMove && !needFile && !needRank) { appendSquare(move.from(), str); }
    }

    template <typename CoordinateType>
    static bool isIdentifiableByType(const Movelist& moves, const Move move, CoordinateType type) {
        static_assert(
            std::is_same_v<CoordinateType, File> || std::is_same_v<CoordinateType, Rank>,
            "CoordinateType must be either File or Rank");

        for (const auto& m : moves) {
            if (m == move || m.to() != move.to()) { continue; }
...
/**
 * Overloads the << operator to print the move's UCI notation.
 *
 * This function is deprecated in the main branch of this library,
 * but we still use it in printing.
 *
 * The main reason for deprecation is that it uses chess960 style
 * UCI notation for castling (e.g. e1h1 instead of e1g1), which
 * causes confusion. But since we are currently not working on this
 * variant, we do not care too much of it.
 *
 * TODO Remove it when better output design is implemented.
 */
inline std::ostream& operator<<(std::ostream& os, const Move& move) {
    os << uci::moveToUci(move, false);
    return os;
}
