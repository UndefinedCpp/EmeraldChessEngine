...
public:
    ...
    void advance() {
        if (buffer_index_ >= bytes_read_) { fill(); }

        ++buffer_index_;
    }

    char peek() {
        if (buffer_index_ + 1 >= bytes_read_) { return stream_.peek(); }
...
public:
    ...
    StreamParserError readGames(Visitor& vis) {
        ...
        if (!stream_buffer.fill()) { return StreamParserError::NotEnoughData; }
        ...
        while (auto c = stream_buffer.some()) {
            if (in_header) {
                ...
                if (*c == '[') {
                    ...
                    if (error != StreamParserError::None) { return error; }
                }
...
private:
        ...
        while (auto c = stream_buffer.some()) {
            switch (*c) {
                ...
                case '"':
                    ...
                    // this
                    if (stream_buffer.current() == '\r') { stream_buffer.advance(); }
    ...
    start:
        ...
        */
        ...
        if (!visitor->skip() && has_comment && !is_termination_symbol) { goto start; }
        ...
        while (auto cd = stream_buffer.some()) {
            ...
            if (*curr == '1') {
                if (peek == '-') {
                    ...
                    for (size_t i = 0; i <= 6; ++i) { stream_buffer.advance(); }
