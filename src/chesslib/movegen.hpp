...
[[nodiscard]] inline std::pair<Bitboard, int> movegen::checkMask(const Board& board, Square sq) {
    ...
    if (!mask) { return {constants::DEFAULT_CHECKMASK, checks}; }
...
[[nodiscard]] inline Bitboard movegen::seenSquares(const Board& board, Bitboard enemy_empty) {
    ...
    while (knights) { seen |= attacks::knight(knights.pop()); }

    while (bishops) { seen |= attacks::bishop(bishops.pop(), occ); }

    while (rooks) { seen |= attacks::rook(rooks.pop(), occ); }
...
[[nodiscard]] inline std::array<Move, 2> movegen::generateEPMove(
    ...
    assert(
        (ep.rank() == Rank::RANK_3 && board.sideToMove() == Color::BLACK) ||
...
template <Color::underlying c>
[[nodiscard]] inline Bitboard movegen::generateCastleMoves(
    const Board& board, Square sq, Bitboard seen, Bitboard pin_hv) noexcept {
