...
public:
    ...
    struct KillerTable {
        ...
        bool has(const Move m) const {
            return m.move() == killer1 || m.move() == killer2;
        }
        void add(const Move m) {
            ...
            if (killer1 == 0) { // Empty slot
                killer1 = code;
            } else if (code != killer1 &&
                       code != killer2) { // A new killer move seen
    ...
    struct HistoryTable {
        ...
        void update(const Move m, const PieceType p, int depth, bool isWhite,
                    bool good) {
...
public:
    ...
    void update(const Position &pos, const Move bestMove,
                const std::vector<uint16_t> &quietMoves,
                const std::vector<uint16_t> &captureMoves, bool isWhite,
                int depth, int ply) {
